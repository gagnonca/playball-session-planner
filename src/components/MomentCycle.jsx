 ...
 *
 * - MomentPills → default selector: a row of buttons (title + subtitle inline)
 * - MomentInfo  → read-only "?" helper: the circle diagram + verbose descriptions
 */
...
/* ------------------------------------------------------------------ */
/* Default selector — row of buttons (title + subtitle inline)         */
/* ------------------------------------------------------------------ */
      ...
      {MOMENTS.map((m) => {
        ...
        return (
          ...
          >
            ...
            <span style={{ fontSize: 13, fontWeight: active ? 700 : 600, letterSpacing: '-0.005em' }}>
              {m.label}
            </span>
            <span style={{ fontSize: 11, fontWeight: 500, color: active ? 'var(--accent)' : 'var(--ink-3)' }}>
              {m.sub}
            </span>
          </button>
        );
...
/* ------------------------------------------------------------------ */
/* "?" helper — read-only circle diagram + verbose descriptions        */
/* ------------------------------------------------------------------ */
...
export function MomentInfo() {
  return (
    ...
    >
      {/* Read-only circle diagram */}
      <div
        aria-hidden
        style={{ position: 'relative', width: '100%', maxWidth: 260, aspectRatio: '1 / 1', margin: '0 auto' }}
      >
        <svg
          viewBox="0 0 100 100"
        ...
        {MOMENTS.map((node) => (
          ...
          >
            <span style={{ fontSize: 16, lineHeight: 1 }}>{node.emoji}</span>
            <span style={{ fontSize: 11.5, fontWeight: 600, letterSpacing: '-0.01em', marginTop: 2 }}>
              {node.label}
            </span>
          </div>
        ))}
      ...
      <p className="text-center mt-3" style={{ fontSize: 12, lineHeight: 1.5, color: 'var(--ink-2)' }}>
        The game moves in a continuous circle — <strong style={{ color: 'var(--ink)' }}>winning the ball</strong>{' '}
        builds your attack, <strong style={{ color: 'var(--ink)' }}>losing the ball</strong> starts your defense.
      </p>
